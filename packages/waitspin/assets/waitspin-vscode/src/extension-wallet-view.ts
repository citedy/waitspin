import type { WalletLedgerEntry, WalletStatus } from "./extension-core";
...
const PUBLISHER_LEVELS_DOCS_URL =
  "https://waitspin.com/docs#publisher-levels-and-limits";
const PUBLISHER_LEVELS_LINK_TOKEN = "__WAITSPIN_PUBLISHER_LEVELS_LINK__";
export const PUBLISHER_LEVELS_DOCS_LINK = `<a href="${PUBLISHER_LEVELS_DOCS_URL}" rel="noopener noreferrer">Publisher levels and limits</a>`;
const WALLET_PAYOUT_DOCS_URL =
...
export function renderLedger(entries: WalletLedgerEntry[]): string {
  if (!entries.length) {
...
function renderPayoutReasons(status: WalletStatus): string {
  const reasonSet = new Set(status.payoutBlockedReasons);
  const reasons: string[] = [];

  if (reasonSet.has("connect_account_missing") || !status.connectConnected) {
    reasons.push(
      "Payout account: Not set up. Add a Stripe Express payout account before withdrawals; this is separate from the VS Code plugin connection. Use Set up payout account below.",
    );
  } else if (reasonSet.has("connect_details_not_submitted")) {
    reasons.push(
      "Payout account: Onboarding is incomplete. Finish the payout account details before withdrawals.",
    );
  } else if (
    reasonSet.has("connect_payouts_not_enabled") ||
    reasonSet.has("payouts_disabled") ||
    !status.payoutsEnabled
  ) {
    reasons.push(
      "Payout account: Waiting for Stripe payouts to be enabled on the connected account.",
    );
  }

  if (
    reasonSet.has("earnings_maturing") ||
    ...
    reasons.push(
      `Earnings: Maturing. ${formatMicroUnits(status.balance.maturingMicroUnits)} is recorded but not withdrawable yet${maturityWindow}.${trustLevel}${nextEligible}`,
    );
  }

  if (reasonSet.has("balance_below_minimum")) {
    const minimum =
      status.minPayoutCents !== undefined
        ? formatCents(status.minPayoutCents)
        : "the minimum payout";
    const payoutable =
      status.payoutTransferCents !== undefined
        ? formatCents(status.payoutTransferCents)
        : formatMicroUnits(
            Math.max(
              status.balance.availableMicroUnits -
                status.balance.reversalDebtMicroUnits,
              0,
            ),
          );
    const reversalDebt =
      status.balance.reversalDebtMicroUnits > 0
        ...
        : "";
    reasons.push(
      `Balance: Below minimum. Payoutable now: ${payoutable}; minimum payout: ${minimum}.${reversalDebt}`,
    );
  }
  ...
  if (reasonSet.has("payout_cadence_cooldown")) {
    ...
    const nextEligible = nextEligibleAt
      ...
      : "";
    reasons.push(
      `Payout timing: Waiting for the next scheduled payout window.${nextEligible}`,
    );
  }

  if (reasonSet.has("reversal_debt_outstanding")) {
    reasons.push(
      "Balance: Previous refunds or reversals must be settled before payout.",
    );
  }

  if (
    reasonSet.has("publisher_quarantined") ||
    reasonSet.has("risk_score_payout_hold") ||
    reasonSet.has("risk_score_quarantine")
  ) {
    reasons.push(
      "Account review: Payouts are paused while WaitSpin reviews publisher risk signals.",
    );
  }
...
function renderPayoutActions(
  status: WalletStatus,
  installId: string | undefined,
): string {
  const reasonSet = new Set(status.payoutBlockedReasons);
  const actionHref =
    reasonSet.has("connect_account_missing") || !status.connectConnected
  ...
  const primary = actionHref
    ? `<a href="${escapeHtml(actionHref)}">${actionLabel}</a> · `
    : "";
  return `<p class="muted">${primary}<a href="${WALLET_PAYOUT_DOCS_URL}">Wallet and payout details</a></p>`;
}

function payoutConnectUrl(installId: string | undefined): string {
...
function renderReasonText(reason: string): string {
  return escapeHtml(reason).replaceAll(
    PUBLISHER_LEVELS_LINK_TOKEN,
    PUBLISHER_LEVELS_DOCS_LINK,
  );
}

function publisherLevelStatusLabel(status: string | undefined): string | undefined {
  if (!status) {
    return undefined;
  }
  switch (status) {
...
function renderPublisherLevelSentence(status: WalletStatus): string {
  if (
    status.publisherTrustLevel === undefined ||
...
function formatOptionalDateTime(value: string | undefined): string | undefined {
  if (!value) {
    return undefined;
  }
